...
# --- GELİŞMİŞ VERİ DÖNGÜSÜ ---
def fiyatlari_guncelle_loop():
    global fiyat_deposu
    # Yahoo'yu kandırmak için gerçek bir tarayıcı gibi davranıyoruz
    ...
    while True:
        try:
            ...
            if semboller:
                # Toplu indirme yerine tek tek deneme (IP engeli için daha güvenli)
                for s in semboller:
                    try:
                        ticker = yf.Ticker(s, session=session)
                        # fast_info bazen Render'da hata verir, history en sağlamıdır
                        ...
                        if not df.empty:
                            ...
                            # Finnhub yedeği
                            if FINNHUB_API_KEY:
                                r = requests.get(f"https://finnhub.io/api/v1/quote?symbol={s}&token={FINNHUB_API_KEY}", timeout=5)
                                if r.status_code == 200:
                                    price = r.json().get('c', 0)
                                    if price > 0: fiyat_deposu[s] = price
                    except: continue
                    time.sleep(1) # IP ban koruması
...
def delete_hisse():
    ...
    s = veriyi_yukle()
    kod = data.get("hisse", "").upper().strip()
    # Hem .IS haliyle hem yalın haliyle kontrol et
